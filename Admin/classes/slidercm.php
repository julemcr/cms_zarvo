...
/**
 * Table class for slidercm
 */
class slidercm extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'slidercm';
		$this->TableName = 'slidercm';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`slidercm`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// SlidercmID
		$this->SlidercmID = new DbField('slidercm', 'slidercm', 'x_SlidercmID', 'SlidercmID', '`SlidercmID`', '`SlidercmID`', 3, -1, FALSE, '`SlidercmID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->SlidercmID->IsAutoIncrement = TRUE; // Autoincrement field
		$this->SlidercmID->IsPrimaryKey = TRUE; // Primary key field
		$this->SlidercmID->Sortable = TRUE; // Allow sort
		$this->SlidercmID->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['SlidercmID'] = &$this->SlidercmID;

		// Titulo
		$this->Titulo = new DbField('slidercm', 'slidercm', 'x_Titulo', 'Titulo', '`Titulo`', '`Titulo`', 200, -1, FALSE, '`Titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Detalle
		$this->Detalle = new DbField('slidercm', 'slidercm', 'x_Detalle', 'Detalle', '`Detalle`', '`Detalle`', 200, -1, FALSE, '`Detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Detalle->Sortable = TRUE; // Allow sort
		$this->fields['Detalle'] = &$this->Detalle;

		// Url_image
		$this->Url_image = new DbField('slidercm', 'slidercm', 'x_Url_image', 'Url_image', '`Url_image`', '`Url_image`', 200, -1, TRUE, '`Url_image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->Url_image->Nullable = FALSE; // NOT NULL field
		$this->Url_image->Required = TRUE; // Required field
		$this->Url_image->Sortable = TRUE; // Allow sort
		$this->Url_image->UploadMaxFileSize = 1024000;
		$this->fields['Url_image'] = &$this->Url_image;

		// Orden
		$this->Orden = new DbField('slidercm', 'slidercm', 'x_Orden', 'Orden', '`Orden`', '`Orden`', 3, -1, FALSE, '`Orden`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Orden->Nullable = FALSE; // NOT NULL field
		$this->Orden->Required = TRUE; // Required field
		$this->Orden->Sortable = TRUE; // Allow sort
		$this->Orden->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['Orden'] = &$this->Orden;

		// Estado
		$this->Estado = new DbField('slidercm', 'slidercm', 'x_Estado', 'Estado', '`Estado`', '`Estado`', 200, -1, FALSE, '`Estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`slidercm`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->SlidercmID->setDbValue($conn->insert_ID());
			$rs['SlidercmID'] = $this->SlidercmID->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('SlidercmID', $rs))
				AddFilter($where, QuotedName('SlidercmID', $this->Dbid) . '=' . QuotedValue($rs['SlidercmID'], $this->SlidercmID->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->SlidercmID->DbValue = $row['SlidercmID'];
	...
	public function deleteUploadedFiles($row)
	{
		...
		$oldFiles = EmptyValue($row['Url_image']) ? [] : [$row['Url_image']];
		foreach ($oldFiles as $oldFile) {
			if (file_exists($this->Url_image->oldPhysicalUploadPath() . $oldFile))
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('SlidercmID', $row) ? $row['SlidercmID'] : NULL) : $this->SlidercmID->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@SlidercmID@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "slidercmedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "slidercmadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("slidercmview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("slidercmview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "slidercmadd.php?" . $this->getUrlParm($parm);
		else
			$url = "slidercmadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("slidercmedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("slidercmadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("slidercmdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "SlidercmID:" . JsonEncode($this->SlidercmID->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->SlidercmID->CurrentValue != NULL) {
			$url .= "SlidercmID=" . urlencode($this->SlidercmID->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("SlidercmID") !== NULL)
				$arKeys[] = Param("SlidercmID");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->SlidercmID->CurrentValue = $key;
	...
	{
		$this->SlidercmID->setDbValue($rs->fields('SlidercmID'));
		$this->Titulo->setDbValue($rs->fields('Titulo'));
		$this->Detalle->setDbValue($rs->fields('Detalle'));
		$this->Url_image->Upload->DbValue = $rs->fields('Url_image');
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->SlidercmID);
					$doc->exportCaption($this->Titulo);
					$doc->exportCaption($this->Detalle);
					$doc->exportCaption($this->Url_image);
		...
		while (!$recordset->EOF && $recCnt < $stopRec) {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						$doc->exportField($this->SlidercmID);
						$doc->exportField($this->Titulo);
						$doc->exportField($this->Detalle);
						$doc->exportField($this->Url_image);
	...
	public function getFileData($fldparm, $key, $resize, $width = THUMBNAIL_DEFAULT_WIDTH, $height = THUMBNAIL_DEFAULT_HEIGHT)
	{
		...
		$fileNameFld = "";
		$fileTypeFld = "";
		if ($fldparm == 'Url_image') {
			$fldName = "Url_image";
			$fileNameFld = "Url_image";
		} else {
		...
		if (count($ar) == 1) {
			$this->SlidercmID->CurrentValue = $ar[0];
		} else {
