...
function CurrentProjectID() {
	...
	return "{5ACD2586-5733-4EB1-9592-B819B3395C82}";
}
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionUserID($rs->fields('PersonID')); // Load User ID
	...
	public function loadUserID()
	{
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($rsUser = $UserTableConn->execute($sql)) {
				while (!$rsUser->EOF) {
					$this->addUserID($rsUser->fields('PersonID'));
					$rsUser->moveNext();
				}
				$rsUser->close();
			}
